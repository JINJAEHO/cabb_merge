...
func NewNode(nodeID string) *Node {
	...
	if nodeID != "10000" {
		http.Post("http://192.168.10.35:9999/newNode", "application/json", buff) //msp로 보냄
	}
...
func (node *Node) Reply(msg *ReplyMsg) error {
	...
	for _, value := range node.CommittedMsgs {
		fmt.Printf("#Committed value# TxID : %x, TimeStamp : %s, WAdr : %s, Applier : %s , Company : %s, Career : %s, Job : %s, Sign : %s \n",
			value.TxID[:], string(value.TimeStamp), value.WAddr, value.Applier, value.Company, value.Career, value.Job, value.Sign[:])
	}
	fmt.Print("\n")
